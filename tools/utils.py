...
def regularization_loss_sparse(emb: torch.Tensor, adj: torch.Tensor, n_neg: int = 5, eps: float = 1e-6) -> torch.Tensor:
    """
    Memory-safe regularization loss for large N.

    Unlike `regularization_loss`, this does NOT build an N×N similarity matrix.
    It uses positive pairs from a sparse adjacency (torch sparse COO) and samples
    random negatives.

    Args:
        emb: (N, D) embedding tensor
        adj: torch sparse adjacency (N, N), typically spatial graph with self-loops
        n_neg: number of negatives per positive edge
        eps: numerical stability epsilon for logs
    ...
    if not adj.is_sparse:
        raise ValueError("regularization_loss_sparse expects a torch sparse adjacency tensor.")

    # Normalize embeddings to compute cosine similarity as dot product.
    emb_n = F.normalize(emb, p=2, dim=1)

    # Positive edges from sparse adjacency indices.
    idx = adj.coalesce().indices()  # (2, nnz)
    src = idx[0]
    dst = idx[1]
    # Drop self loops for this loss.
    mask = src != dst
    src = src[mask]
    dst = dst[mask]
    ...
    pos_sim = (emb_n[src] * emb_n[dst]).sum(dim=1)  # (E,)
    pos_prob = torch.sigmoid(pos_sim).clamp(min=eps, max=1 - eps)
    pos_loss = -torch.log(pos_prob).mean()

    # Sample random negatives for each positive edge (src fixed, dst random).
    n = emb_n.shape[0]
    src_rep = src.repeat_interleave(int(n_neg))
    dst_neg = torch.randint(0, n, (src_rep.shape[0],), device=emb.device)
    neg_sim = (emb_n[src_rep] * emb_n[dst_neg]).sum(dim=1)
    neg_prob = torch.sigmoid(neg_sim).clamp(min=eps, max=1 - eps)
...
def dicr_loss(com1, com2):
    ...
    Args:
        com1: first view embedding (spatial GCN output)
        com2: second view embedding (feature GCN output)
    Returns:
        L_{DICR}
    """
